# 🚀 Crypto Investment Manager

A Python-based crypto portfolio management system that analyzes market data, evaluates risk, predicts future prices, and suggests optimal investment allocation.

---

## 📌 Overview

This project simulates how real-world portfolio managers make investment decisions using:

- Historical market data  
- Risk analysis (volatility)  
- Machine learning prediction (Linear Regression)  
- Portfolio allocation strategies  
- Data visualization (EDA)  

---
## 🧩 Sprint Plan

### Sprint 1 (Weeks 1-2)
- Setup project structure  
- Learn Streamlit and database  
- Implement data fetching  

---

### Sprint 2 (Weeks 3-4)
- Implement risk analysis  
- Add prediction model (Linear Regression)  
- Store data in SQLite  

---

### Sprint 3 (Weeks 5-6)
- Build UI dashboard  
- Add graphs and visualization  
- Implement portfolio allocation  

---

### Sprint 4 (Weeks 7-8)
- Improve UI design  
- Fix bugs  
- Prepare documentation  

---

## ⚙️ Features

- ✅ Fetch real-time cryptocurrency data (CoinGecko API)  
- ✅ Store and manage data using SQLite  
- ✅ Risk classification (Low / Medium / High)  
- ✅ Future price prediction using Linear Regression  
- ✅ Smart portfolio allocation based on risk + prediction  
- ✅ Interactive dashboard using Streamlit  
- ✅ Data visualization (trends, distribution, allocation)  

---

## 🧠 Technologies Used

- Python  
- Streamlit  
- SQLite  
- NumPy  
- Scikit-learn  
- Matplotlib  

---

## 📊 System Workflow

1. Fetch historical crypto data  
2. Store data in database  
3. Analyze risk using volatility  
4. Predict future prices using ML  
5. Allocate portfolio intelligently  
6. Visualize results in dashboard  

---

## 🚀 How to Run

### 1. Clone repository

```bash
git clone https://github.com/seswanthmotepalli/crypto_portfolio_manager.git
cd crypto_portfolio_manager
